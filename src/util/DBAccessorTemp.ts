...
export class DBAccessorTemp {
    /**
     * 指定したテーブルの全件を取得するサンプルメソッド
     * @param tableName 取得したいテーブル名
     */
    static async selectAll(tableName: string): Promise<Record<string, unknown>[]> {
        ...
        return new Promise((resolve, reject) => {
            ...
            connection.on('connect', (err: any) => {
                if (err) {
                    return cleanupAndReject(new Error(`SQL Server Connect error: ${err}`));
                }

                const request = new Request(`SELECT * FROM [${tableName}]`, (reqErr: any) => {
                    if (reqErr) {
